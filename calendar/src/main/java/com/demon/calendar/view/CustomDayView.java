...
public class CustomDayView extends DayView {
     ...
     * @param context 上下文
     */
    public CustomDayView(Context context, boolean isLunar) {
    ...
    public void refreshContent() {
        ...
        renderMarker(day.getDate());
        super.refreshContent();
    }

    private void renderMarker(CalendarDate date) {
        if (CalendarUtil.loadMarkData().containsKey(date.toString())) {
            marker.setVisibility(VISIBLE);
            String s = CalendarUtil.loadMarkData().get(date.toString());
            marker.setText(s);
        } else {
    ...
    public IDayRenderer copy() {
        return new CustomDayView(context, isLunar);
    }
}
